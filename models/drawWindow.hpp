...
class DrawWindow {
 public:
  ...
  void printEnemyLeftList(pEnemyList enemyList, pEnemyList specialEnemyList,
  ...
  pRoom changeRoom(Character &character, Character &character2,
                   int &normalEnemyCount, int &specialEnemyCount,
                   int &bossEnemyCount, pEnemyList &normalEnemyList,
                   pEnemyList &specialEnemyList, pEnemyList &bossEnemyList,
                   pPosition &mountainList, pPosition &bonusList,
                   pRoom roomList, int &maxRoom, bool multiplayer);

  void howMuchEnemies(int maxRoom, bool multiplayer, int &normalEnemyCount,
                      int &specialEnemyCount, int &bossEnemyCount);
  ...
  void moveEnemyMultiplayer(pEnemyList enemyList, Character character,
                            Character character2, DrawWindow drawWindow,
                            long points);
  void moveEnemySinglePlayer(pEnemyList enemyList, Character character,
